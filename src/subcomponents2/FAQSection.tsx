...
export default function FAQSection() {
    ...
    const [activeAccordionKey, setActiveAccordionKey] = useState<string>('0'); // Set default active accordion item (0 for first item)

    const faqs: Faq[] = [
        {
            question: 'How can I screen mirror under different WiFi?',
            answer: 'With premium account, you can easily screen mirror your device with local network to another. Remote screen mirroring feature of AirDroid will allow you to screen mirror devices which are not under the same internet.'
        },
        {
            question: 'How many devices can I sign in at the same time?',
            answer: 'You can sign in up to 3 devices simultaneously with a standard account. Premium users get unlimited device connections.'
        },
        {
            question: 'Can I use the remote network for screen sharing with a free account?',
            answer: 'Remote network screen sharing is a premium feature. Free accounts can only use screen sharing on the same network.'
        },
        {
            question: 'Do AirDroid Cast and Cast Web have the same features?',
            answer: 'While both share core functionalities, Cast Web has some limitations compared to the desktop application.'
        },
        {
    ...
    const hotTopics: HotTopic[] = [
        ...
        },
        {
            ...
            details: 'Learn how to troubleshoot common screen mirroring issues with AirDroid, including connectivity problems.'
        },
        {
            topic: 'AirDroid Device Management Tips',
            details: 'Get tips on managing multiple devices efficiently with AirDroid, including bulk actions and automated workflows.'
        },
        {
            topic: 'AirDroid Cast vs. AirDroid Web: What’s the Difference?',
            details: 'A deep dive into the features of AirDroid Cast and AirDroid Web, highlighting the pros and cons of each.'
        }
    ];
        ...
        <Container className="py-5">
            ...
            <Nav
                ...
                onSelect={(k: string | null) => {
                    if (k === 'faqs' || k === 'topics') {
                        setActiveTab(k);
                        setActiveAccordionKey('0'); // Reset to first accordion item when changing tabs
                    }
                }}
            >
                <Nav.Item>
            ...
            <Accordion
                ...
                activeKey={activeAccordionKey} // Set the active item for accordion
                onSelect={(selectedKey) => setActiveAccordionKey(selectedKey as string)} // Update active item when accordion item is clicked
            >
